...
import com.hwoss.service.IAuthService;
...
public class AuthController {
    ...
    @ApiOperation(value = "添加权限记录", notes = "设置指定用户对指定文档的权限")
    @ApiImplicitParams({@ApiImplicitParam(name = "files", value = "文档", example = "file1,file2,file3", required = true),
    ...
    @ApiImplicitParams({@ApiImplicitParam(name = "user", value = "用户", required = true), @ApiImplicitParam(name =
            "file", value = "文档", required = true), @ApiImplicitParam(name = "offset", value = "偏移量", required = true)})
    @AuthInterceptor(InterceptorLevel.ADMIN)
