...
Vue.component('file-filter-item', {
    ...
    '                    <input type="text" title="指定文档" data-toggle="tooltip" class="form-control file-filter file-name-filter"' +
    '                           placeholder="文档名" onkeyup="fileFilter();"/>' +
    '                </div>' +
    '            </div>' +
    '            <br/>' +
    '        </div>'
...
function deleteCategory() {
    ...
    $.ajax({
        url: "/category/" + categoryId, type: "DELETE", success: function (data) {
            ...
            alerts("删除失败，该分类已被多个文档引用，无法删除（可尝试修改文档分类或删除文档）。");
        }
    });
}
...
$(document).ready(function () {
    ...
    $(".to-upload-button").click(function () {
        layer.prompt({title: '请输入文档前缀，支持规则', formType: 0}, function (prefix, index) {
            layer.close(index);
